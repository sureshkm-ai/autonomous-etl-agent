...
async def _process_message(sqs, message: dict[str, Any]) -> bool:
    ...
    try:
        ...
        await async_update_run(run_id, status="FAILED", error_message=f"Story validation failed: {exc}")
        await write_audit_event("RUN_FAILED", run_id=run_id, story_id=body.get("story_id"),
    ...
    try:
        ...
        await write_audit_event(event_type, run_id=run_id, story_id=story.id,
                                 ...
                                 payload={"github_pr_url": final_state.get("github_pr_url"),
                                          "s3_artifact_url": final_state.get("s3_artifact_url")})
